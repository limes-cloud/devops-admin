    ...
    <el-card class="box-card">
      <el-form ref="queryForm" :model="queryParams" :inline="true" label-width="68px">
        ...
        <el-form-item label="发布时间">
          <el-date-picker v-model="startandend" size="small" value-format="timestamp" :clearable="false" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="chooseTime" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table v-loading="loading" v-permission="'service:release_log:page'" :data="List">
        ...
        <el-table-column label="发布详情" align="center" width="120px">
          <template slot-scope="scope">
            <el-popover placement="left" width="400" trigger="click">
              <div class="desc-box">
                <div v-for=" (val,index) in getDescList(scope.row.desc)" :key="index" class="desc-item">
                  {{ index+1 }} : {{ val }}
                </div>
              </div>
      ...
      <el-form ref="form" :model="form" label-width="80px" :rules="rules">
        ...
        <el-form-item label="选择镜像" prop="pack_id">
          <el-select v-model="form.pack_id" filterable placeholder="请选择">
            <el-option v-for="(item,index) in images" :key="index" :label="item.image_name+'（'+parseTime(item.created_at)+'）'" :value="item.id" />
          </el-select>
        </el-form-item>

        <el-form-item label="发布环境" prop="env_keyword">
          <el-select v-model="form.env_keyword" placeholder="请选择">
            <el-option v-for="(item,index) in envs" :key="index" :label="item.name" :value="item.keyword" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getServices(name) {
      const srvList = await getServices({ page: 1, count: 10, name: name })
      return srvList
    },
